...
function ec(id){if(window._echarts[id]){window._echarts[id].dispose()}window._echarts[id]=echarts.init(document.getElementById(id),'dark')}
function grad(col){return new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:col},{offset:1,color:'transparent'}])}
function baseOpt(lbs){return{grid:{left:40,right:6,top:8,bottom:18},xAxis:{type:'category',data:lbs,show:true,axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'rgba(240,253,244,0.35)',fontSize:9}},yAxis:{type:'value',show:true,min:0,splitLine:{lineStyle:{color:'rgba(255,255,255,0.06)',type:'dashed'}},axisLabel:{color:'rgba(240,253,244,0.35)',fontSize:9}},animationDuration:300}}

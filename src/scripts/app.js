...
if(node.gpu_name&&node.gpu_name!=='None'&&node.gpu_name!=='-'){leftRows.push({l:'GPU',v:node.gpu_name})}leftRows.push({type:'header',l:'📊 使用率'},{l:'CPU',type:'metric',pct:cpu,cls:'cpu'},{l:'内存',type:'metric',pct:mp,cls:'mem'},{l:'磁盘',type:'metric',pct:dp,cls:'dsk'});var rightRows=[{type:'header',l:'📡 网络'},{l:'流量限额',v:tl>0?bytes(tl):'无'},{type:'header',l:'⚡ 状态'},{l:'进程数',v:latest.process||'-'},{l:'TCP',v:(latest.connections&&latest.connections.tcp)||'-'},{l:'更新',v:age(latest.updated_at)},{l:'到期',v:node.expired_at?new Date(node.expired_at).toLocaleDateString('zh-CN'):'-'}]

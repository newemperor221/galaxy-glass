...
function baseOpt(lbs){return{backgroundColor:'transparent',grid:{left:4,right:4,top:4,bottom:4},xAxis:{type:'category',data:lbs,show:false},yAxis:{type:'value',show:false,min:0},animationDuration:300}}
...
ec('chart-cpu');window._echarts['chart-cpu'].setOption(Object.assign(baseOpt(lbs),{series:[{type:'line',data:cpuPts.map(function(p){return p.r}),smooth:true,symbol:'none',lineStyle:{width:2,color:'#10b981'},areaStyle:{color:grad('rgba(16,185,129,0.12)')}}],tooltip:{trigger:'axis'}}))
ec('chart-mem');window._echarts['chart-mem'].setOption(Object.assign(baseOpt(lbs),{series:[{type:'line',data:memPts.map(function(p){return p.r}),smooth:true,symbol:'none',lineStyle:{width:2,color:'#818cf8'},areaStyle:{color:grad('rgba(129,140,248,0.12)')}}],tooltip:{trigger:'axis'}}))

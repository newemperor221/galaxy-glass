...
var leftRows=[{type:'header',l:'💻 硬件'},{l:'CPU 型号',v:node.cpu_name||'-'},{l:'核心数',v:node.cpu_cores?'× '+node.cpu_cores:'-'},{l:'架构',v:node.arch||'-'},{l:'虚拟化',v:node.virtualization||'-'},{l:'操作系统',v:(node.os||'-').split(' ').slice(0,2).join(' ')},{l:'Swap',v:(node.swap_total||0)>0?bytes(node.swap_total):'无'},{l:'磁盘',v:bytes(node.disk_total)}]
if(node.gpu_name&&node.gpu_name!=='None'&&node.gpu_name!=='-'){leftRows.push({l:'GPU',v:node.gpu_name})};var rightRows=[{type:'header',l:'📡 网络'},{l:'流量限额',v:tl>0?bytes(tl):'无'},{type:'header',l:'⚡ 状态'},{l:'进程数',v:latest.process||'-'},{l:'TCP',v:(latest.connections&&latest.connections.tcp)||'-'},{l:'更新',v:age(latest.updated_at)},{l:'到期',v:node.expired_at?new Date(node.expired_at).toLocaleDateString('zh-CN'):'-'}]
...
function cg(id,l1){var c=document.getElementById(id).getContext('2d'),g=c.createLinearGradient(0,0,0,130);g.addColorStop(0,l1);g.addColorStop(1,'rgba(0,0,0,0)');return g}
function mk(id,cfg){if(window._charts&&window._charts[id]){window._charts[id].data=cfg.data;window._charts[id].update('none')}else{if(!window._charts)window._charts={};window._charts[id]=new Chart(document.getElementById(id).getContext('2d'),cfg)}}
var lbs=cpuPts.map(function(p){return p.t.toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})}),cpuMax=Math.max(100,Math.ceil(Math.max.apply(null,cpuPts.map(function(p){return p.r}))/10)*10),memMax=Math.max(100,Math.ceil(Math.max.apply(null,memPts.map(function(p){return p.r}))/10)*10)
mk('chart-cpu',{type:'line',data:{labels:lbs,datasets:[{data:cpuPts.map(function(p){return p.r}),borderColor:'#10b981',backgroundColor:cg('chart-cpu','rgba(16,185,129,0.12)'),fill:true,tension:0.3,pointRadius:0,pointHitRadius:10,borderWidth:2}]},options:{responsive:true,maintainAspectRatio:false,animation:{duration:300},plugins:{legend:{display:false},tooltip:{callbacks:{label:function(ctx){return ctx.parsed.y.toFixed(1)+'%'}}}},scales:{x:{display:false,grid:{display:false}},y:{display:false,min:0,max:cpuMax,grid:{display:false}}}}})
mk('chart-mem',{type:'line',data:{labels:lbs,datasets:[{data:memPts.map(function(p){return p.r}),borderColor:'#818cf8',backgroundColor:cg('chart-mem','rgba(129,140,248,0.12)'),fill:true,tension:0.3,pointRadius:0,pointHitRadius:10,borderWidth:2}]},options:{responsive:true,maintainAspectRatio:false,animation:{duration:300},plugins:{legend:{display:false},tooltip:{callbacks:{label:function(ctx){return ctx.parsed.y.toFixed(1)+'%'}}}},scales:{x:{display:false,grid:{display:false}},y:{display:false,min:0,max:memMax,grid:{display:false}}}}})
var netAll=[],netLabels=netPts.map(function(p){netAll.push(p.u);netAll.push(p.d);return p.t.toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})}),netMax=Math.max.apply(null,netAll)||1
function bytesLabel(v){if(v>=1073741824)return(v/1073741824).toFixed(1)+'GB/s';if(v>=1048576)return(v/1048576).toFixed(1)+'MB/s';if(v>=1024)return(v/1024).toFixed(1)+'KB/s';return v.toFixed(0)+'B/s'}
mk('chart-net',{type:'line',data:{labels:netLabels,datasets:[{label:'上行',data:netPts.map(function(p){return p.u}),borderColor:'#f59e0b',backgroundColor:cg('chart-net','rgba(245,158,11,0.12)'),fill:true,tension:0.3,pointRadius:0,pointHitRadius:10,borderWidth:2,yAxisID:'y'},{label:'下行',data:netPts.map(function(p){return p.d}),borderColor:'#10b981',backgroundColor:cg('chart-net','rgba(16,185,129,0.12)'),fill:true,tension:0.3,pointRadius:0,pointHitRadius:10,borderWidth:2,yAxisID:'y'}]},options:{responsive:true,maintainAspectRatio:false,animation:{duration:300},plugins:{legend:{display:false},tooltip:{callbacks:{label:function(ctx){return ctx.dataset.label+': '+bytesLabel(ctx.parsed.y)}}}},scales:{x:{display:false,grid:{display:false}},y:{display:false,min:0,max:netMax,grid:{display:false}}}}})}

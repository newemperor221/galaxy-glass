...
var total=nodesList.length,off=total-on;document.querySelectorAll('#stat-online-value').forEach(function(e){e.textContent=on+'/'+total});document.querySelectorAll('#stat-region-value').forEach(function(e){e.innerHTML=(off>0?'<span style="color:var(--danger)">'+off+' 离线</span> · ':'')+'点亮区域 '+Object.keys(rs).length})

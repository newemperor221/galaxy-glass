...
export async function getExchangeRate(): Promise<number> {
  const cacheKey = "usd_cny_rate";
  const cacheTimeKey = "usd_cny_rate_time";
  const CACHE_HOURS = 1;
  const now = Date.now();
  const cached = localStorage.getItem(cacheKey);
  const cachedTime = localStorage.getItem(cacheTimeKey);
  if (cached && cachedTime && now - parseInt(cachedTime) < CACHE_HOURS * 3600000) {
  ...
  try {
    ...
    return cached ? parseFloat(cached) : 7.2;
  }
}
...
export function getOSIcon(os?: string): string {
  ...
  if (lower.includes("alpine")) return "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/alpinejs/alpinejs-original.svg";
  return "";
}
